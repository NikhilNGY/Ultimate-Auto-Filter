...
async def send_page(client, message, chat_id, page: int):
    ...
    if page > 0:
        nav_buttons.append(
            types.InlineKeyboardButton(
                "⏮️ First", callback_data=f"auto_first_0_{chat_id}_{encoded_query}"
            )
        )
        nav_buttons.append(
            types.InlineKeyboardButton(
    ...
    if page < total_pages - 1:
        nav_buttons.append(
            types.InlineKeyboardButton(
                "Next ➡️", callback_data=f"auto_next_{page+1}_{chat_id}_{encoded_query}"
            )
        )
        nav_buttons.append(
            types.InlineKeyboardButton(
                "⏭️ Last",
                callback_data=f"auto_last_{total_pages-1}_{chat_id}_{encoded_query}",
            )
        )
    ...
    sent_msg = await message.reply(
        f"🔎 Search Results (Page {page+1} of {total_pages}):",
...
async def callback(client, callback_query):
    ...
    action, page, chat_id, encoded_query = (
        parts[1],
        int(parts[2]),
        int(parts[3]),
        parts[4],
